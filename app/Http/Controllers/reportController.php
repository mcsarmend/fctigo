    ...
    public function reportesesioncartera(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                " 	LC.ID AS ACUERDO,".
                "     CV.VALUE AS NoTitularCuenta,".
                "     COALESCE (CLI.FIRSTNAME,'') || ' ' || COALESCE (CLI.LASTNAME, '') AS NombreTitulaCuenta,".
                "     CV3.VALUE AS CICLO,".
                "     B.NAME AS SUCURSAL,".
                "     AD.REGION AS ESTADO,".
                "     CASE ".
                " 		WHEN CV2.VALUE IS NULL THEN 'CONTIGO'".
                " 	ELSE ".
                " 		CV2.VALUE".
                " 	END AS Fondeador,".
                "     (".
                "     SELECT sum(LA.principalbalance)".
                " 		FROM mambu_prod.loanaccount as LA".
                " 		WHERE LA.lineofcreditkey = min(LC.encodedkey)".
                " 	) AS SALDO_CAPITAL".
                " FROM mambu_prod.lineofcredit AS LC".
                " 	JOIN mambu_prod.client CLI ON CLI.ENCODEDKEY = LC.CLIENTKEY".
                "     join mambu_prod.customfieldvalue CV ON CV.PARENTKEY = CLI.ENCODEDKEY".
                "     LEFT join mambu_prod.customfieldvalue CV2 ON CV2.PARENTKEY = LC.ENCODEDKEY AND CV2.CUSTOMFIELDKEY = '8a44bf8d7f65a97a017f6bd14aa67d23'".
                "     LEFT JOIN mambu_prod.customfield cf ON cf.ENCODEDKEY = CV2.CUSTOMFIELDKEY and cf.type IN ('LINE_OF_CREDIT')  ".
                "     LEFT join mambu_prod.customfieldvalue CV3 ON CV3.PARENTKEY = LC.ENCODEDKEY AND CV3.CUSTOMFIELDKEY = '8a4422367be572b8017be640f98d5075'".
                "     LEFT JOIN mambu_prod.customfield cf2 ON cf.ENCODEDKEY = CV3.CUSTOMFIELDKEY and cf2.type IN ('Ciclo Grupo')  ".
                "     JOIN mambu_prod.loanaccount LA_B ON LA_B.ACCOUNTHOLDERKEY = LC.CLIENTKEY".
                "     JOIN mambu_prod.branch B ON B.ENCODEDKEY = LA_B.ASSIGNEDBRANCHKEY".
                "     JOIN mambu_prod.address AD ON AD.PARENTKEY = B.ENCODEDKEY".
                " where LC.ID = 'SSP303' group by lc.id, cv.value, cli.firstname, cli.lastname, cv3.value, b.name, ad.region, cv2.value ";
                ...
                for ($i=0; $i < count($join1); $i++) {
                    for ($j=0; $j < count($join2); $j++) {
                        if($join1[$i]->strAcuerdo == $join2[$j]->strAcuerdo){
                            $tmp = [];
                            array_push($tmp,$join2[$j]->acuerdo);
                            array_push($tmp,$join1[$i]->fecha_etiquetado);
                ...
                if($resultado == '')  return []; else return $resultado;
